...
async def sudo(event):
    ...
    if sudo == "True":
        await eor(event, f"GodHackeerzUserbot\nSudo - Enabled\nSudo user(s) - {users}")
    else:
        await eor(event, f"GodHackeerzUserbot\nSudo - Disabled")
...
async def tb(event):
    ...
    if sudousers:
        ...
        newsudo = f"{target}"
    await ok.edit(f"Added {target} as a sudo user. Restarting.. Give me a minute.Master..")
    heroku_var[GodHackeerzUserbot] = newsudo
