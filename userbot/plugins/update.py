...
UPSTREAM_REPO_URL = "https://github.com/rohithaditya/Godhackerz-userbot"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                "`[HEROKU]`\n`Please set up the` **HEROKU_APP_NAME** `variable Master"
                " to be able to deploy Me To Serve You Better...`"
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await event.edit(
                f"{txt}\n" "`Invalid Heroku credentials for deploying userbot dyno Master.`"
            )
            return repo.__del__()
        await event.edit(
            "`[HEROKU]`" "\n`Userbot dyno build in progress, please wait... Master`"
        )
        ups_rem.fetch(ac_br)
        ...
        await event.edit("`Successfully deployed!\n" "Restarting, please wait. 2-3 Minutes Master. I Will Be Soon To serve You Master 😂😂😂..`")
    else:
        await event.edit(
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "`Successfully Updated!\n" "I Will Be Available Again @2-3 Minutes Master. I will be back on To Serve you Master😂😂😂😂`"
    )
...
@borg.on(admin_cmd(outgoing=True, pattern=r"update")) 
@borg.on(pattern="update") 
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`Deploying userbot, please wait....Master`")
        await deploy(event, repo, ups_rem, ac_br, txt)
        return
    if changelog == "" and not force_update:
        await event.edit(
            "\n`GODHACKERZUSERBOT is`  **up-to-date**  `with`  "
